...
async function obtenerContextoNegocio(req) {
  ...
  try {
    ...
    const wPres   = where.replace('AND propietario_id', 'AND pr.propietario_id');
    const wMat    = where;
    const wObraSub = where.replace('AND propietario_id', 'AND o.propietario_id');
    ...
    const [
      ...
      pool.query(`
        SELECT
          ...
          COUNT(*) FILTER (WHERE t.jefe_id IS NULL)::int                               AS jefes,
          COUNT(*) FILTER (WHERE t.jefe_id IS NOT NULL)::int                           AS subordinados,
          COUNT(*) FILTER (WHERE t.estado_id = 1)::int                                 AS activos_administrativo,
          COUNT(*) FILTER (WHERE t.estado_id = 2)::int                                 AS inactivos_administrativo,
          COUNT(*) FILTER (WHERE t.estado_id IS NULL)::int                             AS sin_estado_administrativo,
          COUNT(*) FILTER (
            WHERE NOT EXISTS (
              SELECT 1 FROM labores l2
              WHERE l2.trabajador_id = t.id AND l2.archivado = FALSE
      ...
      pool.query(`
        ...
        FROM (
          ...
          LEFT JOIN presupuestos pr ON pr.labor_id = l.id OR pr.obra_id = o.id
          LEFT JOIN gastos_imprevistos gi ON gi.obra_id = o.id AND gi.estado_id != 15
          WHERE o.archivado = FALSE AND o.estado_id != 22 ${wObraSub}
          GROUP BY o.id
        ) sub
      `, params),
...
── TRABAJADORES ──
- Total: ${tr.total} (${tr.jefes} jefes, ${tr.subordinados} subordinados)
- Estado administrativo: ${tr.activos_administrativo} activos | ${tr.inactivos_administrativo} inactivos | ${tr.sin_estado_administrativo} sin estado
- Sin labores activas (capacidad ociosa): ${tr.sin_labores_activas} de ${tr.total}
...
Para preguntas sobre trabajadores, usá consultar_trabajadores — soporta filtro por especialidad_id (ej. "trabajadores herreros"), estado_id (1=Activo, 2=Inactivo, estado administrativo) y sin_labores (capacidad ociosa). El estado administrativo es independiente de tener o no labores activas — no los confundas si el usuario pregunta por uno u otro.
Para pagos realizados a un trabajador específico, usá consultar_pagos con trabajador_id.

Cada presupuesto separa costo_mano_obra de costo_materiales — son conceptos distintos, nunca los sumes vos manualmente ni asumas que uno incluye al otro. consultar_presupuestos y consultar_costo_total_obra ya te dan ambos por separado más costo_total (la suma). Si el usuario pregunta "cuánto es la mano de obra" o "cuánto son los materiales" de un presupuesto, respondé con el campo específico, no el total.
