...
const TOOLS = [
  ...
  {
    name: 'consultar_trabajadores',
    description: 'Lista trabajadores con asistencia del mes, especialidad y estado administrativo. Soporta filtro por nombre, especialidad, estado administrativo o capacidad ociosa. IMPORTANTE: estado_administrativo (activo/inactivo) es independiente de si el trabajador tiene labores activas — no los confundas.',
    input_schema: {
      type: 'object',
      properties: {
        busqueda:        { type: 'string' },
        especialidad_id: { type: 'integer' },
        estado_id:       { type: 'integer', description: '1=Activo, 2=Inactivo (estado administrativo del trabajador)' },
        sin_labores:     { type: 'boolean', description: 'true para traer solo trabajadores sin labores activas' },
        ordenar_por:     { type: 'string', enum: ['asistencia_asc', 'asistencia_desc', 'nombre'] },
      },
    },
  ...
  {
    name: 'consultar_presupuestos',
    description: 'Lista presupuestos activos con desglose de costo_mano_obra y costo_materiales por separado (más costo_total = suma de ambos). Para filtrar por obra, usá obra_nombre. Nunca asumas que total_estimado incluye ambos conceptos — usá costo_total para eso.',
    input_schema: {
      type: 'object',
      properties: {
        obra_id:     { type: 'integer' },
...
async function consultar_trabajadores({ busqueda, especialidad_id, estado_id, sin_labores, ordenar_por } = {}, req) {
  const { where, params } = getFiltro(req);
  const condiciones = ['1=1'];
  const valores = [...params];
  ...
  if (busqueda)        { valores.push(`%${busqueda}%`); condiciones.push(`(t.nombre ILIKE $${valores.length} OR t.apellido ILIKE $${valores.length})`); }
  if (especialidad_id) { valores.push(especialidad_id); condiciones.push(`t.especialidad_id = $${valores.length}`); }
  if (estado_id)       { valores.push(estado_id); condiciones.push(`t.estado_id = $${valores.length}`); }
  if (sin_labores)     condiciones.push(`NOT EXISTS (SELECT 1 FROM labores l2 WHERE l2.trabajador_id = t.id AND l2.archivado = FALSE AND l2.estado_id NOT IN (14, 2))`);
  ...
  const result = await pool.query(`
    SELECT
      t.id, t.nombre, t.apellido, t.email,
      e.nombre   AS especialidad_nombre,
      est.nombre AS estado_administrativo,
      t.jefe_id,
      ROUND(
        COUNT(DISTINCT DATE(p.fecha))::numeric /
        NULLIF((
    ...
    FROM trabajadores t
    LEFT JOIN especialidades e ON e.id = t.especialidad_id
    LEFT JOIN estados est      ON est.id = t.estado_id
    LEFT JOIN presentismos p ON p.trabajador_id = t.id
      AND DATE(p.fecha) >= DATE_TRUNC('month', CURRENT_DATE)
    LEFT JOIN labores l ON l.trabajador_id = t.id AND l.archivado = FALSE
    WHERE ${condiciones.join(' AND ')} ${where.replace('AND propietario_id', 'AND t.propietario_id')}
    GROUP BY t.id, e.nombre, est.nombre
    ORDER BY ${orden} LIMIT 50
  `, valores);
...
async function consultar_presupuestos({ obra_id, obra_nombre, estado_id, en_borrador } = {}, req) {
  ...
  const presupuestoIds = result.rows.map(p => p.id);
  const costoMaterialesPorId = {};

  if (presupuestoIds.length > 0) {
    const materialesResult = await pool.query(`
      SELECT pm.presupuesto_id,
             COALESCE(SUM(pm.cantidad * pm.precio_unitario), 0)::numeric AS costo_materiales
      FROM presupuesto_materiales pm
      WHERE pm.presupuesto_id = ANY($1)
      GROUP BY pm.presupuesto_id
    `, [presupuestoIds]);
    materialesResult.rows.forEach(r => {
      costoMaterialesPorId[r.presupuesto_id] = Number(r.costo_materiales);
    });
  }

  return result.rows.map(p => {
    const costo_materiales = costoMaterialesPorId[p.id] ?? 0;
    const costo_mano_obra  = Number(p.costo_mano_obra ?? 0);
    return {
      ...p,
      costo_materiales,
      costo_mano_obra,
