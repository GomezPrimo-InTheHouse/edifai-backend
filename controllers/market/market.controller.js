...
const enviarMensaje = async (req, res) => {
  ...
  try {
    ...
    const tx = txResult.rows[0];
    const esParteDelChat = tx.comprador_id === req.user.userId || tx.vendedor_id === req.user.userId;
    ...
    const destinatarioId = req.user.userId === tx.vendedor_id ? tx.comprador_id : tx.vendedor_id;
    await notificar({
      tipo: 'market_mensaje',
