...
//     // ── Restricción: solo una vez por día por obra ──────────
//     const yaMarco = await client.query(
//       `SELECT id FROM presentismos
...
//     if (yaMarco.rows.length > 0) {
//       await client.query('ROLLBACK');
//       return res.status(409).json({
//         success: false,
//         message: 'Ya marcaste presentismo en esta obra hoy',
//       });
//     }
...
//     // ── Sumar 3 puntos ──────────────────────────────────────
//     const puntosActualizados = await client.query(
//       `UPDATE trabajadores SET puntos = puntos + 3, updated_at = NOW()
//        WHERE id = $1 RETURNING puntos`,
//       [trabajador.id]
//     );
//     const puntosActuales = puntosActualizados.rows[0].puntos;

//     // ── Verificar presentismos esta semana (lunes a viernes) ─
//     const presentismosSemana = await client.query(
//       `SELECT COUNT(DISTINCT DATE(fecha)) AS dias
//        FROM presentismos
//        WHERE trabajador_id = $1
//          AND DATE(fecha) >= DATE_TRUNC('week', CURRENT_DATE)
//          AND DATE(fecha) <= DATE_TRUNC('week', CURRENT_DATE) + INTERVAL '4 days'`,
//       [trabajador.id]
//     );
//     const diasSemana = parseInt(presentismosSemana.rows[0].dias);
...
//     if (diasSemana === 3) {
//       await notificar({
//         tipo: 'bonus_presentismo',
//         mensaje: `🏅 ¡Bien hecho ${trabajador.nombre}! Llevás 3 días esta semana. ¡Seguí así!`,
//         usuario_id: userId,
//       });
//     }

//     // ── Bonus: semana completa (5 días lunes a viernes) ─────
//     if (diasSemana === 5) {
//       await notificar({
//         tipo: 'bonus_semana_completa',
//         mensaje: `🏆 ¡Increíble ${trabajador.nombre}! Completaste toda la semana. ¡Sos un crack!`,
//         usuario_id: userId,
//       });
//     }
...
//       GROUP BY t.id, t.nombre, t.apellido, t.puntos
...
//       SELECT
//         t.id, t.nombre, t.apellido, t.puntos,
...
//     const rankingPuntosResult = await pool.query(`
...
//         ranking_puntos:      rankingPuntosResult.rows,
...
// GET /presentismo/mis-obras — sin cambios (es del trabajador logueado)
const getMisObras = async (req, res) => {
  const userId = req.user.userId;

  try {
    const trabajadorResult = await pool.query(
      `SELECT id, nombre, apellido FROM trabajadores WHERE usuario_id = $1`, [userId]
    );

    if (trabajadorResult.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'No se encontró un trabajador vinculado a este usuario' });
    }
...
// POST /presentismo/marcar — sin cambios (es del trabajador logueado)
const marcarPresentismo = async (req, res) => {
  const userId = req.user.userId;
  const { obra_id, latitud, longitud, observaciones } = req.body;

  if (!obra_id || latitud == null || longitud == null) {
    return res.status(400).json({ success: false, message: 'Faltan campos obligatorios: obra_id, latitud, longitud' });
  }
  ...
  try {
    ...
    const trabajadorResult = await client.query(
      `SELECT id, nombre, apellido, puntos FROM trabajadores WHERE usuario_id = $1`, [userId]
    );
    ...
    const yaMarco = await client.query(
      `SELECT id FROM presentismos WHERE trabajador_id = $1 AND obra_id = $2 AND DATE(fecha) = CURRENT_DATE`,
      [trabajador.id, obra_id]
    );

    if (yaMarco.rows.length > 0) {
      await client.query('ROLLBACK');
      return res.status(409).json({ success: false, message: 'Ya marcaste presentismo en esta obra hoy' });
    }

    const vinculacion = await client.query(
      `SELECT id FROM trabajadores_obras WHERE trabajador_id = $1 AND obra_id = $2 AND (fecha_hasta IS NULL OR fecha_hasta >= CURRENT_DATE)`,
      [trabajador.id, obra_id]
    );

    if (vinculacion.rows.length === 0) {
      await client.query('ROLLBACK');
      return res.status(403).json({ success: false, message: 'No tenés vinculación activa con esta obra' });
    }
    ...
    const puntosActualizados = await client.query(
      `UPDATE trabajadores SET puntos = puntos + 3, updated_at = NOW() WHERE id = $1 RETURNING puntos`,
      [trabajador.id]
    );
    const puntosActuales = puntosActualizados.rows[0].puntos;

    const presentismosSemana = await client.query(
      `SELECT COUNT(DISTINCT DATE(fecha)) AS dias FROM presentismos
       WHERE trabajador_id = $1
         AND DATE(fecha) >= DATE_TRUNC('week', CURRENT_DATE)
         AND DATE(fecha) <= DATE_TRUNC('week', CURRENT_DATE) + INTERVAL '4 days'`,
      [trabajador.id]
    ...
    if (diasSemana === 5) await notificar({ tipo: 'bonus_semana_completa', mensaje: `🏆 ¡Increíble ${trabajador.nombre}! Completaste toda la semana. ¡Sos un crack!`, usuario_id: userId });

    res.status(201).json({ success: true, data: result.rows[0], puntos: puntosActuales, dias_semana: diasSemana });

  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Error en marcarPresentismo:', error.message);
    await notificar({ tipo: 'error_sistema', mensaje: `Error al marcar presentismo: ${error.message}`, usuario_id: null });
    res.status(500).json({ success: false, message: 'Error interno del servidor' });
  } finally {
    client.release();
  }
...
// GET /presentismo/historial — sin cambios (es del trabajador logueado)
const getHistorial = async (req, res) => {
  const userId = req.user.userId;

  try {
    const trabajadorResult = await pool.query(`SELECT id FROM trabajadores WHERE usuario_id = $1`, [userId]);
    if (trabajadorResult.rows.length === 0)
      return res.status(404).json({ success: false, message: 'Trabajador no encontrado' });
    ...
    const result = await pool.query(
      `SELECT p.*, o.nombre AS obra_nombre FROM presentismos p
       LEFT JOIN obras o ON o.id = p.obra_id
       WHERE p.trabajador_id = $1 ORDER BY p.fecha DESC LIMIT 50`,
      [trabajadorId]
    );
...
// GET /presentismo/estadisticas — filtrado por propietario
const getEstadisticas = async (req, res) => {
  const { fecha_desde, fecha_hasta, obra_id } = req.query;
  ...
  // Filtro adicional para admin_privado
  const filtroTrabajador = esAdminPrivado
    ? `AND t.propietario_id = ${propietarioId}`
    : '';
  const filtroObra = esAdminPrivado
    ? `AND o.propietario_id = ${propietarioId}`
    : '';
  ...
  try {
    const resumenResult = await pool.query(`
      SELECT
        COUNT(*)                          AS total_registros,
        COUNT(DISTINCT p.trabajador_id)   AS trabajadores_activos,
        COUNT(DISTINCT p.obra_id)         AS obras_con_actividad,
        COUNT(DISTINCT DATE(p.fecha))     AS dias_con_actividad
      FROM presentismos p
      JOIN trabajadores t ON t.id = p.trabajador_id
      WHERE DATE(p.fecha) BETWEEN $1 AND $2
    ...
    const ausentesResult = await pool.query(`
      ...
      JOIN obras o ON o.id = tob.obra_id
      WHERE (tob.fecha_hasta IS NULL OR tob.fecha_hasta >= CURRENT_DATE)
        AND t.id NOT IN (SELECT DISTINCT trabajador_id FROM presentismos WHERE DATE(fecha) = CURRENT_DATE)
        ${obra_id ? 'AND tob.obra_id = $1' : ''}
        ${filtroTrabajador}
      ORDER BY t.id, t.apellido
    `, obra_id ? [obra_id] : []);
    ...
    const porDiaResult = await pool.query(`
      SELECT
        ...
        TO_CHAR(p.fecha, 'Day')   AS nombre_dia,
        COUNT(*) AS total
      FROM presentismos p
      JOIN trabajadores t ON t.id = p.trabajador_id
      WHERE DATE(p.fecha) BETWEEN $1 AND $2
    ...
    const diasPeriodo = Math.ceil((new Date(hasta) - new Date(desde)) / (1000 * 60 * 60 * 24)) + 1;

    const perfectosResult = await pool.query(`
      SELECT t.id, t.nombre, t.apellido, t.puntos,
             COUNT(DISTINCT DATE(p.fecha)) AS dias_asistidos
      FROM presentismos p
      JOIN trabajadores t ON t.id = p.trabajador_id
      WHERE DATE(p.fecha) BETWEEN $1 AND $2
    ...
    const rankingPuntosResult = await pool.query(`
      SELECT id, nombre, apellido, puntos
      FROM trabajadores
      WHERE puntos > 0 ${esAdminPrivado ? `AND propietario_id = ${propietarioId}` : ''}
...
// GET /presentismo/historial-admin — filtrado por propietario
const getHistorialAdmin = async (req, res) => {
  const { obra_id, trabajador_id, fecha } = req.query;
  const esAdminPrivado = req.user.rol_id === ROL_ADMIN_PRIVADO;
  const propietarioId = req.user.userId;
  ...
  try {
    ...
    if (esAdminPrivado) {
      query += ` AND t.propietario_id = $${idx++}`;
