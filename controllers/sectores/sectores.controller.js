...
const TIPOS_VALIDOS = ['piso', 'ala', 'modulo', 'unidad', 'sector', 'otro'];
...
const crearSector = async (req, res) => {
  try {
    const { obra_id, tipo, valor, orden = 0 } = req.body;
    ...
    const result = await pool.query(`
      INSERT INTO sectores (obra_id, tipo, valor, orden, propietario_id)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
...
const actualizarSector = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE id = $4
      RETURNING *
    `, [tipo ?? null, valor ?? null, orden ?? null, id]);
