...
const crearGastoImprevisto = async (req, res) => {
  ...
  try {
    ...
    for (const fp of formas_pago) {
      await client.query(
        `INSERT INTO gastos_imprevistos_formas_pago (gasto_id, forma_pago_id, monto) VALUES ($1,$2,$3)`,
        [gastoId, fp.forma_pago_id, fp.monto]
      );
    }
    ...
    const resPagador = await pool.query(
      `SELECT usuario_id FROM trabajadores WHERE id = $1
       UNION SELECT id AS usuario_id FROM usuarios WHERE id = $1`,
      [pagador_id]
    );
    if (resPagador.rows[0]?.usuario_id) {
      await notificar({
...
const obtenerGastosImprevistos = async (req, res) => {
  try {
    ...
    if (esWorker) {
      ...
      // Si tiene jefe_id, el jefe del equipo es jefe_id; si no, es él mismo
      const jefeId = jefe_id ?? tid;

      // Traer todo el equipo: el jefe + todos los subordinados de ese jefe
      const resEquipo = await pool.query(`
        SELECT id FROM trabajadores
        WHERE id = $1 OR jefe_id = $1
      `, [jefeId]);
    ...
    } else if (esAdminPrivado) {
      whereExtra = `AND gi.propietario_id = $1`;
    ...
    const data = result.rows.map(g => ({ ...g, formas_pago: formasPagoMap[g.id] ?? [] }));
    return res.status(200).json({ success: true, data });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Error al obtener gastos imprevistos', error: error.message });
  }
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT gi.*,
       ...
       LEFT JOIN formas_pago    fp  ON fp.id  = gi.forma_pago_id
       LEFT JOIN estados        est ON est.id = gi.estado_id
       WHERE gi.obra_id = $${params.length + 1} AND gi.estado_id != 15
...
const obtenerGastoImprevistoPorId = async (req, res) => {
  try {
    ...
    if (esWorker) {
      const resTrabajador = await pool.query(`SELECT id FROM trabajadores WHERE usuario_id = $1`, [req.user.userId]);
      if (resTrabajador.rows.length === 0)
        return res.status(403).json({ success: false, message: 'No autorizado' });
    ...
    if (esAdminPrivado && gasto.propietario_id !== req.user.userId)
      return res.status(403).json({ success: false, message: 'Sin permiso sobre este gasto' });
    ...
    return res.status(200).json({ success: true, data: { ...gasto, formas_pago: resFP.rows } });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Error al obtener el gasto imprevisto', error: error.message });
  }
};
...
const actualizarEstadoGasto = async (req, res) => {
  try {
    ...
    if (req.user.rol_id !== 1 && req.user.rol_id !== ROL_ADMIN_PRIVADO)
      return res.status(403).json({ success: false, message: 'Sin permisos para cambiar el estado de un gasto imprevisto' });

    if (!estado_id)
      return res.status(400).json({ success: false, message: 'Debe especificar estado_id', faltantes: ['estado_id'] });

    const resEstado = await pool.query(
      `SELECT id, nombre FROM estados WHERE id = $1 AND ambito = 'gastos imprevistos'`, [estado_id]
    );
    if (resEstado.rows.length === 0)
      return res.status(404).json({ success: false, message: 'El estado especificado no existe o no corresponde a gastos imprevistos' });
    ...
    const resGasto = await pool.query(
      `SELECT gi.id, gi.monto, gi.pagado_por_id, gi.propietario_id,
              t.usuario_id AS pagador_usuario_id
       FROM gastos_imprevistos gi
       LEFT JOIN trabajadores t ON t.id = gi.pagado_por_id
       WHERE gi.id = $1 AND gi.estado_id != 15`,
    ...
    const gasto = resGasto.rows[0];
    if (req.user.rol_id === ROL_ADMIN_PRIVADO && gasto.propietario_id !== req.user.userId)
      return res.status(403).json({ success: false, message: 'Sin permiso sobre este gasto' });
    ...
    await notificar({ tipo: 'gasto_imprevisto_estado', mensaje: `Gasto imprevisto #${id} cambió a estado "${estadoNombre}"`, usuario_id: null });
    ...
    return res.status(200).json({ success: true, message: 'Estado actualizado con éxito', data: result.rows[0] });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Error al actualizar el estado', error: error.message });
  }
};
...
const eliminarGastoImprevisto = async (req, res) => {
  try {
    ...
    if (req.user.rol_id !== 1 && req.user.rol_id !== ROL_ADMIN_PRIVADO)
      return res.status(403).json({ success: false, message: 'Sin permisos para eliminar un gasto imprevisto' });

    const resGasto = await pool.query(
      `SELECT id, propietario_id FROM gastos_imprevistos WHERE id = $1 AND estado_id != 15`, [id]
    );
    if (resGasto.rows.length === 0)
      return res.status(404).json({ success: false, message: 'Gasto imprevisto no encontrado o ya eliminado' });

    if (req.user.rol_id === ROL_ADMIN_PRIVADO && resGasto.rows[0].propietario_id !== req.user.userId)
      return res.status(403).json({ success: false, message: 'Sin permiso sobre este gasto' });

    await pool.query(`UPDATE gastos_imprevistos SET estado_id = 15 WHERE id = $1`, [id]);
    return res.status(200).json({ success: true, message: 'Gasto imprevisto eliminado correctamente' });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Error al eliminar el gasto imprevisto', error: error.message });
  }
...
const actualizarDeudorGasto = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE gastos_imprevistos SET deudor_cliente_id = $1, deudor_usuario_id = $2, updated_at = NOW() WHERE id = $3 RETURNING *`,
      [deudor_cliente_id ?? null, deudor_usuario_id ?? null, id]
    );
...
const subirTicket = async (req, res) => {
  ...
  try {
    ...
    const { data } = supabase.storage.from('ticket-gasto').getPublicUrl(fileName);
    return res.status(200).json({ success: true, url: data.publicUrl });
  } catch (err) {
    console.error('Error en subirTicket:', err);
    return res.status(500).json({ success: false, message: 'Error interno del servidor' });
