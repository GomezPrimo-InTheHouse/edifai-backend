...
const getAllPagos = async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN presupuestos pr ON p.presupuesto_id = pr.id
      LEFT JOIN formas_pago  fp ON p.forma_pago_id  = fp.id
      WHERE 1=1 ${where.replace('AND propietario_id', 'AND p.propietario_id')}
      ORDER BY p.created_at DESC
    `, params);
    res.status(200).json({ success: true, data: result.rows });
  } catch (error) {
...
const getPagoById = async (req, res) => {
  ...
  try {
    ...
    const pago = result.rows[0];
    if (req.user.rol_id === ROL_ADMIN_PRIVADO && pago.propietario_id !== req.user.userId)
      return res.status(403).json({ success: false, message: 'Sin permiso sobre este pago' });
...
const getPagosByTrabajador = async (req, res) => {
  ...
  try {
    ...
    const pagosResult = await pool.query(`
      ...
      LEFT JOIN presupuestos pr ON p.presupuesto_id = pr.id
      LEFT JOIN formas_pago  fp ON p.forma_pago_id  = fp.id
      WHERE p.trabajador_id = $${params.length + 1}
    ...
    const presupuestosResult = await pool.query(`
      ...
      FROM presupuestos pr
      LEFT JOIN labores l ON l.id = pr.labor_id
      WHERE pr.estado_id = 5 AND l.trabajador_id = $1
    `, [trabajador_id]);

    const totalManoObra  = presupuestosResult.rows.reduce((acc, p) => acc + Number(p.costo_mano_obra ?? 0), 0);
    const totalPagado    = pagos.filter(p => p.estado === 'Pagado').reduce((acc, p) => acc + Number(p.monto), 0);
    const totalParcial   = pagos.filter(p => p.estado === 'Parcial').reduce((acc, p) => acc + Number(p.monto), 0);
    const totalCobrado   = totalPagado + totalParcial;
...
const getPagosByPresupuesto = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN trabajadores t ON p.trabajador_id = t.id
      LEFT JOIN formas_pago fp ON p.forma_pago_id = fp.id
      WHERE p.presupuesto_id = $${params.length + 1}
      ${where.replace('AND propietario_id', 'AND p.propietario_id')}
      ORDER BY p.fecha DESC
    ...
    const totalPresupuesto = Number(presupuesto.rows[0]?.total_estimado ?? 0);
    const totalPagado = result.rows.filter(p => p.estado === 'Pagado').reduce((acc, p) => acc + Number(p.monto), 0);

    res.status(200).json({
      success: true,
      data: result.rows,
      saldo_pendiente:   totalPresupuesto - totalPagado,
      total_presupuesto: totalPresupuesto,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error interno del servidor' });
...
const createPago = async (req, res) => {
  ...
  try {
    const presupuestoResult = await pool.query(`SELECT id, estado_id FROM presupuestos WHERE id = $1`, [presupuesto_id]);
    if (presupuestoResult.rows.length === 0)
      return res.status(404).json({ success: false, message: 'El presupuesto no existe' });

    if (presupuestoResult.rows[0].estado_id !== 5)
      return res.status(400).json({ success: false, message: 'Solo se pueden registrar pagos en presupuestos con estado Confirmado' });

    const result = await pool.query(`
      INSERT INTO pagos (presupuesto_id, trabajador_id, monto, fecha, motivo, forma_pago_id, estado, propietario_id)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *
    `, [presupuesto_id, trabajador_id, monto, fecha, motivo ?? null, forma_pago_id ?? null, estado ?? 'Pendiente', propietario_id]);

    const trabajadorResult = await pool.query(`SELECT usuario_id, nombre, apellido FROM trabajadores WHERE id = $1`, [trabajador_id]);
    const trabajador = trabajadorResult.rows[0];

    await notificar({
      tipo: 'pago_realizado',
      mensaje: `Se registró un pago de $${monto} para ${trabajador?.nombre ?? ''} ${trabajador?.apellido ?? ''}`,
      usuario_id: trabajador?.usuario_id ?? null,
    });

    res.status(201).json({ success: true, data: result.rows[0] });
  } catch (error) {
    console.error('Error al crear pago:', error.message);
    notificar({ tipo: 'error_sistema', mensaje: `Error al crear pago: ${error.message}`, usuario_id: null });
    res.status(500).json({ success: false, message: 'Error interno del servidor' });
  }
};
...
const updatePago = async (req, res) => {
  ...
  try {
    const existente = await pool.query(`SELECT propietario_id FROM pagos WHERE id = $1`, [id]);
    ...
    await notificar({ tipo: 'pago_modificado', mensaje: `Pago #${id} fue modificado`, usuario_id: null });
    res.status(200).json({ success: true, data: result.rows[0] });
  } catch (error) {
    notificar({ tipo: 'error_sistema', mensaje: `Error al actualizar pago #${id}: ${error.message}`, usuario_id: null });
    res.status(500).json({ success: false, message: 'Error interno del servidor' });
  }
};
...
const cambiarEstadoPago = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE pagos SET estado=$1, updated_at=NOW() WHERE id=$2 RETURNING *`, [estado, id]
    );

    const pago = result.rows[0];
    const trabajadorResult = await pool.query(`SELECT usuario_id FROM trabajadores WHERE id = $1`, [pago.trabajador_id]);
    const usuarioIdTrabajador = trabajadorResult.rows[0]?.usuario_id ?? null;

    await notificar({ tipo: 'pago_estado', mensaje: `Tu pago #${id} cambió a estado: ${estado}`, usuario_id: usuarioIdTrabajador });
    res.status(200).json({ success: true, data: pago });
  } catch (error) {
    notificar({ tipo: 'error_sistema', mensaje: `Error al cambiar estado de pago #${id}: ${error.message}`, usuario_id: null });
    res.status(500).json({ success: false, message: 'Error interno del servidor' });
  }
};
...
const deletePago = async (req, res) => {
  ...
  try {
    ...
    await pool.query(`DELETE FROM pagos WHERE id = $1`, [id]);
    await notificar({ tipo: 'pago_eliminado', mensaje: `Pago #${id} fue eliminado`, usuario_id: null });
    res.status(200).json({ success: true, message: 'Pago eliminado.' });
  } catch (error) {
    notificar({ tipo: 'error_sistema', mensaje: `Error al eliminar pago #${id}: ${error.message}`, usuario_id: null });
    res.status(500).json({ success: false, message: 'Error interno del servidor' });
  }
};
...
const getEstadisticasPagos = async (req, res) => {
  try {
    const { where, params } = getFiltro(req);
    const w = where.replace('AND propietario_id', 'AND p.propietario_id');
    ...
    const porMesResult = await pool.query(`
      SELECT
        ...
        COUNT(*)   AS cantidad
      FROM pagos p
      WHERE fecha >= NOW() - INTERVAL '12 months' ${w}
      GROUP BY TO_CHAR(fecha, 'YYYY-MM'), TO_CHAR(fecha, 'Mon YY')
      ORDER BY mes ASC
    `, params);
...
module.exports = {
  getAllPagos, getPagoById, createPago, updatePago,
  deletePago, cambiarEstadoPago, getPagosByTrabajador,
  getPagosByPresupuesto, getEstadisticasPagos,
};