...
const vincularTrabajador = async (req, res) => {
  ...
  try {
    ...
    const trabajadorResult = await client.query(
      `SELECT id, nombre, apellido, usuario_id FROM trabajadores WHERE id = $1`, [trabajador_id]
    );
    if (trabajadorResult.rowCount === 0) {
      await client.query('ROLLBACK');
      return res.status(404).json({ success: false, message: 'Trabajador no encontrado' });
    ...
    // Vinculación permanente proveedor externo → trabajador
    await client.query(
      `UPDATE proveedores_externos SET trabajador_id = $1 WHERE id = $2`,
      [trabajador_id, id]
    );
    ...
    if (labor_id) {
      ...
      const laborResult = await client.query(
        `SELECT id, nombre, obra_id, fecha_inicio_estimada FROM labores WHERE id = $1`,
        [labor_id]
      );
      laborData = laborResult.rows[0] ?? null;

      // ── FIX: vincular en tablas intermedias ─────────────────
      await client.query(`
        INSERT INTO labores_trabajadores (labor_id, trabajador_id)
        VALUES ($1, $2) ON CONFLICT DO NOTHING
      `, [labor_id, trabajador_id]);
      ...
      if (laborData?.obra_id) {
        ...
        await vincularEquipoAObra(client, trabajador_id, laborData.obra_id, fechaDesde);
      }
    }

    await client.query('COMMIT');

    // ── FIX: notificar al trabajador asignado ─────────────────
    if (laborData && trabajador.usuario_id) {
      await notificar({
        tipo: 'labor_asignada',
        mensaje: `Se te asignó la labor "${laborData.nombre}".`,
