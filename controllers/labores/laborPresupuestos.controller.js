...
const agregarPresupuesto = async (req, res) => {
  const { labor_id } = req.params;
  const { trabajador_id, proveedor_externo_id, precio, plazo_dias, calidad, garantia, notas, notificar_trabajador } = req.body;
  const client = await pool.connect();
  try {
    if (!precio) return res.status(400).json({ success: false, message: 'El campo precio es obligatorio' });
    ...
    const result = await client.query(`
      INSERT INTO labor_presupuestos
        (labor_id, trabajador_id, proveedor_externo_id, precio, plazo_dias, calidad, garantia, notas, notificar_trabajador)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
      RETURNING *
    `, [labor_id, trabajador_id || null, proveedor_externo_id || null, precio, plazo_dias || null, calidad || null, garantia || null, notas || null, notificar_trabajador ?? false]);
...
module.exports = { listarPresupuestos, agregarPresupuesto, seleccionarPresupuesto, eliminarPresupuesto };