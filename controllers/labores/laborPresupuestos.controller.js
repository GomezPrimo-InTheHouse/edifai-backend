...
const { notificar } = require('../../helpers/notificar.js');
const { ROL_ADMIN_PRIVADO } = require('../../middlewares/filtrarPorPropietario.js');
const { vincularEquipoAObra } = require('../labores/labores.controller.js');
...
const listarPresupuestos = async (req, res) => {
  ...
  const client = await pool.connect();
  try {
    const { error, message } = await verificarAccesoLabor(client, labor_id, req.user);
    if (error) return res.status(error).json({ success: false, message });
...
const seleccionarPresupuesto = async (req, res) => {
  ...
  try {
    ...
    // Datos completos de la labor (incluye fecha para vincular a obra)
    const laborResult = await client.query(
      `SELECT id, nombre, obra_id, propietario_id, fecha_inicio_estimada
       FROM labores WHERE id = $1`,
      [presupuesto.labor_id]
    );
    const laborData = laborResult.rows[0];

    // Resolver cotizante + especialidad a heredar
    let cotizanteNombre = null;
    let especialidadTrabajador = null;
    if (presupuesto.trabajador_id) {
      const tr = await client.query(
    ...
    // ── FIX: vincular en tablas intermedias ───────────────────
    // Sin esto la labor no aparece en el dashboard del trabajador
    // (getDashboardTrabajador filtra por labores_trabajadores)
    // ni la obra aparece como obra_actual (sale de trabajadores_obras)
    if (presupuesto.trabajador_id) {
      await client.query(`
        INSERT INTO labores_trabajadores (labor_id, trabajador_id)
        VALUES ($1, $2) ON CONFLICT DO NOTHING
      `, [presupuesto.labor_id, presupuesto.trabajador_id]);

      if (laborData.obra_id) {
        const fechaDesde = laborData.fecha_inicio_estimada
          ? new Date(laborData.fecha_inicio_estimada).toISOString().split('T')[0]
          : new Date().toISOString().split('T')[0];

        await vincularEquipoAObra(client, presupuesto.trabajador_id, laborData.obra_id, fechaDesde);
      }
    }
    ...
    // ── FIX: notificar al trabajador ganador ──────────────────
    let usuarioGanadorId = null;
    if (presupuesto.trabajador_id) {
      const ganadorResult = await client.query(
        `SELECT usuario_id FROM trabajadores WHERE id = $1`, [presupuesto.trabajador_id]
      );
      usuarioGanadorId = ganadorResult.rows[0]?.usuario_id ?? null;
    }
    ...
    // Notificación dirigida al trabajador asignado
    if (usuarioGanadorId) {
      await notificar({
        tipo: 'labor_asignada',
        mensaje: `Se te asignó la labor "${laborData.nombre}". Tu presupuesto fue confirmado.`,
        usuario_id: usuarioGanadorId,
      });
    }

    // Notificación global para admins
    await notificar({
      tipo: 'presupuesto_seleccionado',
      mensaje: `Presupuesto confirmado para labor "${laborData.nombre}"`,
      usuario_id: null,
    });
