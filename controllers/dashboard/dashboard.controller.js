...
const getDashboardAdmin = async (req, res) => {
  ...
  const fwm = esAdminPrivado ? `AND propietario_id = ${propietarioId}` : '';
    ...
    const [
      ...
      pool.query(`
        SELECT
          ...
          (SELECT COUNT(*) FROM trabajadores WHERE estado_id = 1 ${fw}) AS total_trabajadores
        FROM presentismos
      ...
      pool.query(`
        ...
        WHERE t.estado_id = 1
          ...
          ${esAdminPrivado ? `AND t.propietario_id = ${propietarioId}` : ''}
      ...
      pool.query(`
        ...
        FROM pagos p
        WHERE fecha >= NOW() - INTERVAL '6 months' ${fwp}
...
const getDashboardTrabajador = async (req, res) => {
  ...
  try {
    ...
    const [
      laboresResult, pagosResult, asistenciaMesResult,
    ...
    const diasMarcados = asistenciaMesResult.rows.map(r => new Date(r.dia).toISOString().split('T')[0]);
