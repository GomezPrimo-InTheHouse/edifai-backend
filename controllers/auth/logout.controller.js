...
const logout = async (req, res) => {
  ...
  try {
    // Cerrar SOLO la sesión del refreshToken específico
    const updateResult = await pool.query(
      `UPDATE sesiones
      ...
      [estadoInactivoId, refreshToken]
    );

    if (updateResult.rowCount === 0) {
      return res.status(200).json({
        message: 'No había sesión activa para ese token, pero se procesó el logout igualmente'
      });
    }
